...
function DataFrame::collectTrials(behMsg,DataFrame::df)
    ...
    while size(trialStarts,1) != size(trialStops,1)
        if length(trialStarts) > length(trialStops)
            lengthDifference = length(trialStops) - length(trialStarts)
            firstBadTrial = findall(x->x==0, trialStarts[1:(end-lengthDifference)] .< trialStops)[1]
            deleteat!(trialStops, firstBadTrial)
        else #length(trialStops) > length(trialStarts)
            lengthDifference = length(trialStops) - length(trialStarts)
            firstBadTrial = findall(x->x==0, trialStops[1:(end-lengthDifference)] .> trialStarts)[1]
            deleteat!(trialStops, firstBadTrial)
        end 
    end 
